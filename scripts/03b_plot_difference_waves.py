...
Generate ERP difference-wave plots for the LEMON Emotional Affect analysis.
...
Planned outputs:
  reports/erp_diff/*.png
  reports/erp_diff/*_grand_average.csv
  reports/erp_diff/difference_wave_subject_summary.csv

Usage:
  python scripts/03b_plot_difference_waves.py --config configs/config.yaml
  python scripts/03b_plot_difference_waves.py --config configs/config.yaml --subjects 200 201 202
"""
...
def get_output_root(cfg: dict) -> Path:
    # Previous pipeline writes to ./output by default.
    # Support both "output_root" and fallback "output".
...
SCALP_ROIS: Dict[str, List[str]] = {
    # Posterior / temporo-occipital face-sensitive ROI
    "face_roi": ["P7", "P8", "TP7", "TP8", "O1", "O2"],

    # Central/frontocentral ROI for general task-related effects
    "central_roi": ["Fz", "Cz", "Pz"],

    # Late centro-parietal ROI
...
def build_condition_masks(meta: pd.DataFrame, contrast: str) -> Tuple[np.ndarray, np.ndarray, str, str]:
...
def compute_subject_difference(
    ...
    """Return condition-A minus condition-B ROI waveform for one subject."""
    ...
    data = ep_roi.get_data()  # n_epochs x n_channels x n_times

    # Average across selected ROI channels and epochs.
    # Use nanmean to tolerate remaining NaN channels/samples if any.
...
def main():
    ...
    parser.add_argument("--output-dir", default="reports/erp_diff")
